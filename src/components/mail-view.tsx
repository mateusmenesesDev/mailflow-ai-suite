...
import { mockEmails, initials, avatarColor, type Email } from "@/lib/mock-data";
import { cn } from "@/lib/utils";

export function MailView({ title, emails }: { title: string; emails: Email[] }) {
    ...
    return (
      <div className="flex h-svh flex-col">
        ...
        <div className="flex flex-1 flex-col items-center justify-center text-muted-foreground">
          ...
          <p className="mt-4 text-sm">Nada por aqui.</p>
        </div>
      </div>
    );
  }
      ...
      <section className="flex min-w-0 flex-col border-b md:w-[380px] md:border-b-0 md:border-r xl:w-[420px]">
        <header className="flex items-center gap-2 border-b px-3 py-2.5">
          ...
          <div className="relative flex-1">
            ...
            <Input placeholder={`Buscar em ${title.toLowerCase()}…`} className="h-8 pl-8 text-xs" />
          </div>
          <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0">
            <SlidersHorizontal className="h-3.5 w-3.5" />
          </Button>
          <Button size="sm" className="h-8 gap-1.5 px-3" onClick={() => setComposerOpen(true)}>
        ...
        <Tabs defaultValue="all" className="border-b px-3 py-2">
          <TabsList className="h-8 bg-transparent p-0">
            <TabsTrigger value="all" className="h-7 px-3 text-xs">{title}</TabsTrigger>
            ...
            {selected.hasAttachment && (
              <div className="mt-8">
                ...
                <div className="flex items-center gap-3 rounded-lg border bg-muted/30 p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium">relatorio-mensal.pdf</p>
